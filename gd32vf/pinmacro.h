...
#define GPIO_config(descr) \
  do{ \
    ...
    if(_marg4(descr) == GPIO_PULL){ \
      if(_marg3(descr))GPIO_BC(GPIO(_marg1(descr))) = (1<<_marg2(descr)); \
        else GPIO_BOP(GPIO(_marg1(descr))) = (1<<_marg2(descr)); \
      /*GPIO_BOP(GPIO(_marg1(descr))) = (1<<_marg2(descr)<<((_marg3(descr))*16));*/ \
    } \
  }while(0)
...
#define GPO_ON(descr) \
  do{ \
    GPIO_BOP(GPIO(_marg1(descr))) = (1<<_marg2(descr)<<((1-_marg3(descr))*16)); \
  }while(0)
...
#define GPO_T(descr) \
  do{ \
    GPIO_OCTL(GPIO(_marg1(descr))) ^= (1<<_marg2(descr)); \
  }while(0)
#define GPI_ON(descr)  ((GPIO_ISTAT(GPIO(_marg1(descr))) & (1<<_marg2(descr)))==(_marg3(descr)<<_marg2(descr)))
#define GPI_OFF(descr) ((GPIO_ISTAT(GPIO(_marg1(descr))) & (1<<_marg2(descr)))!=(_marg3(descr)<<_marg2(descr)))
