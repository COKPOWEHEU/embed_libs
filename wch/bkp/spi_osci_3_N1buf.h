...
В данной версии библиотеки выделяется (N+1) буфер, один из которых (попавший
на границу) отбрасывается. Соответственно, значительная часть ОЗУ расходуется
впустую
*/
...
uint8_t spiosci_buf[ (SPIOSCI_channels + 1) * SPIOSCI_SZ ];
...
void spiosci_init(){
  ...
  dma_cfg_io(SPI_DMA_TX(ch1), &SPI_DATA(ch1), spiosci_buf, SPIOSCI_SZ/2);
  ...
  uint32_t bufaddr[4];
  bufaddr[0] = (uint32_t)spiosci_buf;
  bufaddr[1] = bufaddr[0] + SPIOSCI_SZ;
  if( (bufaddr[1] ^ bufaddr[0])>>16 ){bufaddr[0]=bufaddr[1]; bufaddr[1] += SPIOSCI_SZ;}
  bufaddr[2] = bufaddr[1] + SPIOSCI_SZ;
  if( (bufaddr[2] ^ bufaddr[1])>>16 ){bufaddr[1]=bufaddr[2]; bufaddr[2] += SPIOSCI_SZ;}
  bufaddr[3] = bufaddr[2] + SPIOSCI_SZ;
  if( (bufaddr[3] ^ bufaddr[2])>>16 ){bufaddr[2]=bufaddr[3]; bufaddr[3] += SPIOSCI_SZ;}
  
  dma_cfg_io(SPI_DMA_RX(ch1), bufaddr[0], &SPI_DATA(ch1), SPIOSCI_SZ/2);
  ...
  dma_cfg_io(SPI_DMA_RX(ch2), bufaddr[1], &SPI_DATA(ch2), SPIOSCI_SZ/2);
...
#define chbuf1(i)	((uint8_t*)bufaddr[0])[i]
#if ch2 != 0
  #define chbuf2(i)	((uint8_t*)bufaddr[1])[i]
#else
  #define chbuf2(i)	0
#endif
#if ch3 != 0
...
void spiosci_out(spiosci_outfunc_t outfunc){
  ...
  bufaddr[0] = (uint32_t)spiosci_buf;
  bufaddr[1] = bufaddr[0] + SPIOSCI_SZ;
  if( (bufaddr[0] ^ bufaddr[1])>>16 ){bufaddr[0]=bufaddr[1]; bufaddr[1] += SPIOSCI_SZ;}
  bufaddr[2] = bufaddr[1] + SPIOSCI_SZ;
  if( (bufaddr[1] ^ bufaddr[2])>>16 ){bufaddr[1]=bufaddr[2]; bufaddr[2] += SPIOSCI_SZ;}
  bufaddr[3] = bufaddr[2] + SPIOSCI_SZ;
  if( (bufaddr[2] ^ bufaddr[3])>>16 ){bufaddr[2]=bufaddr[3]; bufaddr[3] += SPIOSCI_SZ;}
