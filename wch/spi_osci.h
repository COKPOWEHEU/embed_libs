...
#include "spi.h"
#undef SPIn
#define chbuf1(i) spiosci_buf[0][i]
void dma_register(SPI_DMA_RX(ch1)){}
  ...
  #include "spi.h"
  #undef SPIn
  #define chbuf2(i) spiosci_buf[1][i]
...
uint8_t spiosci_buf[SPIOSCI_channels][SPIOSCI_SZ];

#define TIM	10,1,TIMO_PWM_NINV | TIMO_NEG | TIMO_REMAP1
#define SCK1	A,5,1, GPIO_APP50
#include "timer.h"
...
void spiosci_init(){
  ...
  dma_cfg_io(SPI_DMA_RX(ch1), &spiosci_buf[0], &SPI_DATA(ch1), SPIOSCI_SZ/2);
  ...
  dma_cfg_io(SPI_DMA_RX(ch2), &spiosci_buf[1], &SPI_DATA(ch2), SPIOSCI_SZ/2);
  ...
  dma_cfg_io(SPI_DMA_RX(ch3), &spiosci_buf[2], &SPI_DATA(ch3), SPIOSCI_SZ/2);
  ...
  timer_init(SPIOSCI_TIMER, SPIOSCI_SPEED-1, 2-1);
